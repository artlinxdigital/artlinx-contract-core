 ...
 *
 */
contract ERC721ArtlinxUserFactory is Ownable {
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) external {
        bytes memory data = abi.encodeWithSelector(ERC721ArtlinxUser(0).__ERC721ArtlinxUser_init.selector, _name, _symbol, baseURI, contractURI, operators);
        BeaconProxy beaconProxy = new BeaconProxy(address(beacon), data);
        ERC721ArtlinxUser token = ERC721ArtlinxUser(address(beaconProxy));
