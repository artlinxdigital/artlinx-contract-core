 ...
 *
 */
contract ERC721ArtlinxFactory is Ownable {
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) external {
        bytes memory data = abi.encodeWithSelector(ERC721Artlinx(0).__ERC721Artlinx_init.selector, _name, _symbol, baseURI, contractURI);
        BeaconProxy beaconProxy = new BeaconProxy(address(beacon), data);
        ERC721Artlinx token = ERC721Artlinx(address(beaconProxy));
