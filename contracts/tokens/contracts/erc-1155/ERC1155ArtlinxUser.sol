...
contract ERC1155ArtlinxUser is OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155Lazy, HasContractURI {

    event CreateERC1155ArtlinxUser(address owner, string name, string symbol);

    string public name;
    string public symbol;

    function __ERC1155ArtlinxUser_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) external initializer {
        __ERC1155ArtlinxUser_init_unchained(_name, _symbol, baseURI, contractURI, operators);
        emit CreateERC1155ArtlinxUser(_msgSender(), _name, _symbol);
    }

    function __ERC1155ArtlinxUser_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) internal {
        ...
        __ERC1155ArtlinxUser_init_unchained(_name, _symbol);
        _setBaseURI(baseURI);
    ...
    function __ERC1155ArtlinxUser_init_unchained(string memory _name, string memory _symbol) internal initializer {
        name = _name;
        symbol = _symbol;
    }
