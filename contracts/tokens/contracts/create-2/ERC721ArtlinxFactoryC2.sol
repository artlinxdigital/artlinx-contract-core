...
import "../erc-721-minimal/ERC721ArtlinxMinimal.sol";
 ...
 *
 */
contract ERC721ArtlinxFactoryC2 is Ownable {
    address public beacon;
    address transferProxy;
    address lazyTransferProxy;

    event Create721ArtlinxProxy(address proxy);
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) pure internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721ArtlinxMinimal(0).__ERC721Artlinx_init.selector, _name, _symbol, baseURI, contractURI);
    }
}
