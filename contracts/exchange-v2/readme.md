## Artlinx protocol Exchange smart contracts

### Architecture

Artlinx exchange smart contracts are built using openzeppelin's upgradeable smart contracts library. So the smart contract code can be updated to support new features, fix bugs etc.
...
- ArtlinxTransferManager (sophisticated version, it takes in account protocol commissions, royalties etc)

TODO: There are plans to extend ArtlinxTransferManager to support more royalty schemes and add new features like custom fees, multiple order beneficiaries.
...
ArtlinxTransferManager supports these types of fees:
...
TODO: currently, there are no indexed fields in events, because artlinx protocol uses internal indexing. Possibly, indexed fields will be added in future.  